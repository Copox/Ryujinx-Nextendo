...
namespace Ryujinx.HLE.HOS
{
    public class ModLoader
    {
        ...
        // [Nextendo] Splatoon 3 — MODS AUTORISES (modification locale).
        //
        // A l'origine, ce titre refusait tout mod : le test en ligne se joue sur les serveurs
        // Nextendo contre de vrais adversaires, et un mod (portée d'arme, cadence, vitesse, un
        // simple fichier remplacé par LayeredFS) suffit à gâcher la partie des autres — le serveur
        // ne peut pas voir un client modifié. Parce que le propriétaire de ce build le demande
        // explicitement, cette restriction a été levée : le chargeur de mods collecte et applique
        // normalement romfs, romfs.bin, exefs, correctifs IPS/pchtxt et codes de triche.
        //
        // ⚠️ Les deux correctifs de CONNECTIVITÉ intégrés (NextendoS3Patches : contournement du
        // certificat épinglé et nom de pair) restent appliqués d'office, indépendamment des mods —
        // sans eux le jeu ne peut pas parler aux serveurs. Ils ne viennent pas du disque et ne se
        // confondent pas avec un mod.
        public const ulong Splatoon3ApplicationId = 0x0100C2500FC20000;

        /// <summary>Vrai si ce titre refuse tout mod.</summary>
        /// <summary>
        /// Toujours false sur ce build : l'interdiction de mods de Splatoon 3 a été levée à la
        /// demande du propriétaire. La garde est conservée pour qu'un futur retour en arrière
        /// n'éteigne pas par accident les correctifs de connectivité appliqués dans
        /// <see cref="ApplyNsoPatches"/>.
        /// </summary>
        public static bool ModsInterdits(ulong applicationId) => false;
        ...
        {
            // Splatoon 3 : les correctifs de connectivite (contournement du certificat epingle et
            // nom de pair) sont INTEGRES au binaire (NextendoS3Patches) et doivent s'appliquer dans
            // tous les cas, mods autorises ou non — sans eux, le jeu ne parvient pas a parler a nos
            // serveurs (le TLS s'arrete au ClientHello). On les applique donc AVANT tout le reste.
            bool applique = false;

            if (applicationId == Splatoon3ApplicationId)
            {
                applique = AppliquerCorrectifsIntegres(programs);
            }

            // ⚠️ Si des mods sont interdits pour ce titre, on s'arrete apres les correctifs integres :
            // aucun correctif NSO venant du disque (mods/exefs_patches globaux ou exefs applicatif)
            // ne doit s'appliquer. Aujourd'hui ModsInterdits est inerte (false), mais la garde reste
            // pour ne pas reticentrer une interdiction sans couper les correctifs de connectivite.
            if (ModsInterdits(applicationId))
            {
                return applique;
            }
            ...
            return ApplyProgramPatches(nsoMods, 0x100, programs) | applique;
        }
